<?php
require 'core/database/connection.php';
// var_dump($_GET['id']);
$id = $_GET['id'];
$selectQuery = "select * from bookdetails where id = $id";
$statement = $pdo->query($selectQuery);
$data = $statement->fetch(); 
// echo "<pre>";
// var_dump($data);
// print_r($data);



?>


<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-GLhlTQ8iRABdZLl6O3oVMWSktQOp6b7In1Zl3/Jr59b6EGGoI1aFkw7cmDA6j6gD" crossorigin="anonymous">
    <link rel="stylesheet" href="public/index.css">

</head>

<body>
    <nav class="navbar navbar-expand-lg bg-body-tertiary">
        <div class="container-fluid">
            <a class="navbar-brand" href="#">Virtual Library</a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse"
                data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false"
                aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                    <li class="nav-item dropdown">
                    <a type="button" class="btn btn-primary mx-2" href="index.php">Back</a>
                    </li>
                </ul>
                <form class="d-flex" role="search">
                    <!-- <a type="button" class="btn btn-primary mx-2" href="index.php">Back</a> -->
                    <a type="button" class="btn btn-primary mx-2" href="edit.php?id=<?php echo $data['id'] ?>">Edit</a>
                    <a type="button" class="btn btn-danger mx-2" href="delete.php?id=<?php echo $data['id'] ?>">Delete</a>
                </form>
            </div>
        </div>
    </nav>
    <!-- <h1>this is read more page </h1> -->
    <form action="addBooks.php" class=" my-3 container" method="POST" enctype="multipart/form-data">
        <div class="formClass my-5">
            <div class="mb-3 container imageSection">
                <!-- <input id="uploadFile" onchange="getImagepreview(event)" class="imageInput" type="file"
                    name="imageupload"> -->
                    <img class="readMoreImage" src="<?php echo $data['img_source'] ?>" alt="bookImage">
                <div id="preview"></div>
            </div>
            <div class="mb-3 container inputFeilds">
                <h1>Book name</h1>
                <p><?php echo $data['book_name'] ?></p>
                <h1>Author</h1>
                <p><?php echo $data['author'] ?></p>
                <h1>Description</h1>
                <p><?php echo $data['description'] ?></p>
            </div>
        </div>
        <!-- <div class="btnSection">
            <input type="submit" name="submitted" value="save" class="btn btn-primary saveBtn my-3">
        </div> -->
    </form>

    <script src="public/index.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha2/dist/js/bootstrap.bundle.min.js"
        integrity="sha384-qKXV1j0HvMUeCBQ+QVp7JcfGl760yU08IQ+GpUo5hlbpg51QRiuqHAJz8+BrxE/N" crossorigin="anonymous">
    </script>
</body>

</html>